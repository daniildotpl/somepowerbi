...
SELECT 
    ...
    ROUND(visits/(SELECT population.population FROM population WHERE population.region=statinfo.region AND population.year=statinfo.year)*100000, 1) visits_on_th
FROM statinfo
)
SELECT 
    year AS " | YEAR",
